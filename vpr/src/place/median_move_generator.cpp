...
std::vector<int>& X_coord, std::vector<int>& Y_coord, int &, int place_high_fanout_net) {
    ...
    for (ClusterPinId pin_id : cluster_ctx.clb_nlist.block_pins(b_from)) {
        ...
        if (cluster_ctx.clb_nlist.net_sinks(net_id).size() < 4){
            get_non_updateable(net_id, &new_coords, b_from);
            //VTR_LOG("small net\n");
        }
        else {
            ...
            if(!update_bb(net_id, &new_coords, xold, yold, xnew, ynew)){
                get_non_updateable(net_id, &new_coords, b_from);
            }
            //VTR_LOG("large net (%d,%d), (%d,%d)\n", xnew, ynew, xold, yold);
            //VTR_LOG("(%d,%d,%d,%d),(%d,%d,%d,%d)\n", coords.xmin, coords.xmax, coords.ymin, coords.ymax, new_coords.xmin, new_coords.xmax, new_coords.ymin, new_coords.ymax);
        }
        /* 
        VTR_ASSERT(new_coords.xmin == coords.xmin);
        VTR_ASSERT(new_coords.ymin == coords.ymin);
        VTR_ASSERT(new_coords.xmax == coords.xmax);
        VTR_ASSERT(new_coords.ymax == coords.ymax);
        */
        X_coord.push_back(new_coords.xmin);
        X_coord.push_back(new_coords.xmax);
        Y_coord.push_back(new_coords.ymin);
        Y_coord.push_back(new_coords.ymax);
    }
